$(document).ready(function () {
    function newGame() {
        $(".character").remove();

        userCharacter = "";
        currentEnemy = "";
        finalEnemy = false;
        enemyHistory = [];

        var obiWan = {
            name: "Obi Wan Kenobi",
            imageSource: "https://pm1.narvii.com/6387/ca77b00de2ba14beea7e9c7a498b38e2ac868bce_128.jpg",
            healthPoints: 120,
            attackCount: 0,
            originalAttackPower: 10,
            currentAttackPower: 10,
            counterAttackPower: 23
        };

        var luke = {
            name: "Luke Skywalker",
            imageSource: "data:image/jpeg;base64,",
            healthPoints: 100,
            attackCount: 0,
            originalAttackPower: 9,
            currentAttackPower: 9,
            counterAttackPower: 25
        };

        var darthSidious = {
            name: "Darth Sidious",
            imageSource: "https://yt3.ggpht.com/-BcpKsKGUFk0/AAAAAAAAAAI/AAAAAAAAAAA/cD-RXrzBW3s/s128-c-k-no-mo-rj-c0xffffff/photo.jpg",
            healthPoints: 150,
            attackCount: 0,
            originalAttackPower: 4,
            currentAttackPower: 18,
            counterAttackPower: 12
        };

        var darthMaul = {
            name: "Darth Maul",
            imageSource: "https://pm1.narvii.com/6965/7412a5ab6908fa6a64d6a12484fbf7fbd26bc142r1-1014-1111v2_128.jpg",
            healthPoints: 180,
            attackCount: 0,
            originalAttackPower: 5,
            currentAttackPower: 12,
            counterAttackPower: 10
        };
        //Above objects contain original character data

        character = {
            obiWan: obiWan,
            luke: luke,
            darthSidious: darthSidious,
            darthMaul: darthMaul
        }; //global character object populated with original character values

        var obiWanDiv = $('<div class="col-md-3 character" id="obiWan" data-character="obiWan"><div ><h4 id="obiWanName"></h4><img id="obiWanImage" class="img-thumbnail"><p>Health: <span id="obiWanHealth"></span></p></div>');
        $("#playerOptions").prepend(obiWanDiv);
        var lukeDiv = $('<div class="col-md-3 character" id="luke" data-character="luke"><h4 id="lukeName"></h4><img id="lukeImage" class="img-thumbnail"><p>Health: <span id="lukeHealth"></span></p></div>');
        $("#playerOptions").prepend(lukeDiv);
        var darthSidiousDiv = $('<div class="col-md-3 character" id="darthSidious" data-character="darthSidious"><h4 id="darthSidiousName"></h4><img id="darthSidiousImage" class="img-thumbnail"><p>Health: <span id="darthSidiousHealth"></span></p></div>');
        $("#playerOptions").prepend(darthSidiousDiv);
        var darthMaulDiv = $('<div class="col-md-3 character" id="darthMaul" data-character="darthMaul"><h4 id="darthMaulName"></h4><img id="darthMaulImage" class="img-thumbnail"><p>Health: <span id="darthMaulHealth"></span></p></div>');
        $("#playerOptions").prepend(darthMaulDiv);
        //Above: creates div to be added for each character and prepends to playerOptions
        $("#message").text("Select a player for yourself!");
    }; //Function to be run at start of new game

    var character; //character object to be manipulated as attacks occure

    var userCharacter;
    var currentEnemy;

    var winCount = 0;
    var lossCount = 0;

    var enemyHistory = [];
    var finalEnemy; //indicates when all possible enemies have been added to enemyHistory, final enemy is currentEnemy

    $(document).on("click", '.character', function (event) { //Not sure why document is needed, must research more...
        if (userCharacter === "") {
            $(".character").appendTo("#enemiesAvailable");
            userCharacter = this.getAttribute("data-character");
            characterID = "#" + this.id;
            $(characterID).appendTo("#userCharacter");
            $("#message").text("Select an enemy!");
        } else if (userCharacter !== "" && currentEnemy === "" && this.getAttribute("data-character") !== userCharacter) {
            currentEnemy = this.getAttribute("data-character");
            console.log(currentEnemy);
            enemyHistory.push(this.getAttribute("data-character"));
            if (enemyHistory.length === Object.keys(character).length - 1) {
                finalEnemy = true;
            }; //flags finalEnemy to know when win occurs
            characterID = "#" + this.id;
            $(characterID).appendTo("#currentEnemy");
            $("#message").text("Fight!");
        }
    })

    $("#attack").on("click", function (event) {
        if (character[userCharacter].healthPoints > 0) {
            if (userCharacter !== "" && currentEnemy !== "") {
                $("#message").text("You attacked " + character[currentEnemy].name + " for " + character[userCharacter].currentAttackPower + " damage. " + character[currentEnemy].name + " attacked back for " + character[currentEnemy].counterAttackPower + ". ");
                character[currentEnemy].healthPoints = character[currentEnemy].healthPoints - character[userCharacter].currentAttackPower;
                character[userCharacter].attackCount += 1;
                character[userCharacter].currentAttackPower = character[userCharacter].currentAttackPower + (character[userCharacter].originalAttackPower * character[userCharacter].attackCount);
                character[userCharacter].healthPoints = character[userCharacter].healthPoints - character[currentEnemy].counterAttackPower;
                pageContent();
                if (character[userCharacter].healthPoints <= 0) {
                    lossCount++;
                    $(".character").prependTo("#playerOptions");
                    newGame();
                    pageContent();
                    alert("Game over! Try again!"); //Action to be taken if attack results in user loss
                } else if (character[currentEnemy].healthPoints < 0 && finalEnemy === false) {
                    characterID = "#" + currentEnemy;
                    $(characterID).remove();
                    currentEnemy = "";
                    characterID = ""; //Action to be taken when current enmy is defeated but they are not FINAL enemy
                } else if (character[currentEnemy].healthPoints < 0 && finalEnemy === true) {
                    characterID = "#" + currentEnemy;
                    $(characterID).remove();
                    currentEnemy = "";
                    characterID = "";
                    winCount++;
                    newGame();
                    pageContent();
                } //Action to be taken when current FINAL enemy is defeated, win
            }
        }
    })

    function pageContent() {
        $("#obiWanHealth").text(character.obiWan.healthPoints);
        $("#obiWanName").text(character.obiWan.name);
        $("#obiWanImage").attr('src', character.obiWan.imageSource);
        $("#lukeHealth").text(character.luke.healthPoints);
        $("#lukeName").text(character.luke.name);
        $("#lukeImage").attr('src', character.luke.imageSource);
        $("#darthSidiousHealth").text(character.darthSidious.healthPoints);
        $("#darthSidiousName").text(character.darthSidious.name);
        $("#darthSidiousImage").attr('src', character.darthSidious.imageSource);
        $("#darthMaulHealth").text(character.darthMaul.healthPoints);
        $("#darthMaulName").text(character.darthMaul.name);
        $("#darthMaulImage").attr('src', character.darthMaul.imageSource);
        $("#winCount").text("Wins: " + winCount);
        $("#lossCount").text("Losses: " + lossCount);
    };

    window.onload = newGame();
    window.onload = pageContent();
})